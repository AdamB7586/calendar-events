...
class Events extends Pupils
{
     ...
     * @var array
     */
    public $events = ['Driving Lesson', 'Theory Test', 'Driving Test', 'Other', 'Draft/Provisional Booking', 'Home Visit', 'Cancellation'];
     ...
     */
    protected $colors = [
        1 => ['background' => '#058dc7'/* Blue */],
        2 => ['background' => '#77ab13'/* Green */],
        3 => ['background' => '#ff3333'/* Red */],
        4 => ['background' => '#ffcc33'/* Yellow */, 'font' => '#000000'],
        5 => ['background' => '#4d4d4d'/* Black */],
        6 => ['background' => '#cc6633'/* Brown */],
        7 => ['background' => '#ff9933'/* Orange */, 'font' => '#000000'],
    ...
    public function addLessonEvent($fino, $pupilID, $event, $date, $type)
    {
        ...
        return $this->db->insert($this->getEventsTable(), ['fino' => $fino, 'pupil' => $pupilID, 'event' => $event, 'start' => $dates['startdate'], 'end' => $dates['enddate'], 'type' => $type]);
    }
    ...
    {
        $dates = $this->getEventStartEndDates($date['date'], $date['start_hour'], $date['start_min'], $date['length_hour'], $$date['length_min']);
        return $this->db->update($this->getEventsTable(), ['pupil' => $pupil, 'event' => $event, 'start' => $dates['startdate'], 'end' => $dates['enddate'], 'type' => $type], ['id' => $eventID, 'fino' => $fino]);
    }
    ...
    {
        return $this->db->delete($this->getEventsTable(), ['fino' => $fino, 'id' => $lessonID]);
    }
    ...
    {
        return $this->db->update($this->getEventsTable(), ['pupil' => intval($newUserID)], ['pupil' => intval($origUserID)]);
    }
    ...
    public function getLessonEvents($fino = 0, $id = 0, $startdate = '', $enddate = '', $type = 0, $pupil = 0, $order = 'ASC')
    {
        ...
        if ($pupil != 0 && is_numeric($pupil)) {
            $where['pupil'] = intval($pupil);
        }
    ...
    {
        return $this->db->selectAll($this->getEventsTable(), [$type => intval($userID)]);
    }
